...
# ------------------------------------------------------------
# A3: pure scale / symmetric tail causality
# ------------------------------------------------------------
cfg_a3 <- expand.grid(
  design = "A3",
  m = c(200, 500),
  T = c(1, 2, 5),
  c_val = c(0.10, 0.25, 0.50),
  break_frac = c(0.25, 0.50, 0.75),
  stringsAsFactors = FALSE
)
cfg_a3$instrument_type <- "z"
...
run_simulation_grid(
  ...
  eprocess_tau_grid = c(0.05, 0.10, 0.90, 0.95),
  eprocess_feature = "z",
  nrep = nrep,
  ncores = ncores,
  gamma_vec = c(0, 0.15),
  cvm_weights = c("U", "Late", "Early", "Mid"),
  output_dir = file.path("output", "table3_a3_quantile"),
  file_stub = "table3_a3_quantile"
)
...
run_simulation_grid(
  ...
  gamma_vec = c(0, 0.15),
  cvm_weights = c("U", "Late", "Early", "Mid"),
  output_dir = file.path("output", "table3_a3_expectile"),
  file_stub = "table3_a3_expectile"
)

# ------------------------------------------------------------
# A4: downside-tail causality -- asymmetric instrument and lower-tail e-process
# ------------------------------------------------------------
cfg_a4 <- expand.grid(
  design = "A4",
  m = c(200, 500),
...
run_simulation_grid(
  ...
  eprocess_feature = "zminus",
  nrep = nrep,
  ncores = ncores,
  gamma_vec = c(0, 0.15),
  cvm_weights = c("U", "Late", "Early", "Mid"),
  output_dir = file.path("output", "table3_a4_quantile"),
  file_stub = "table3_a4_quantile"
)
...
run_simulation_grid(
  ...
  gamma_vec = c(0, 0.15),
  cvm_weights = c("U", "Late", "Early", "Mid"),
  output_dir = file.path("output", "table3_a4_expectile"),
  file_stub = "table3_a4_expectile"
)
