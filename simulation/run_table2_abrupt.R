...
cfg_abrupt$instrument_type <- "z"

# Quantile monitoring + e-process benchmark
run_simulation_grid(
  config_grid = cfg_abrupt,
  model_type = "quantile",
  ...
  output_dir = file.path("output", "table2_abrupt_quantile"),
  file_stub = "table2_abrupt_quantile"
)
...
run_simulation_grid(
  ...
  gamma_vec = c(0, 0.15),
  cvm_weights = c("U", "Late", "Early", "Mid"),
  output_dir = file.path("output", "table2_abrupt_expectile"),
  file_stub = "table2_abrupt_expectile"
)
